...
public class ChatBuilder {
    ...
    public ChatBuilder(CooldownManager cooldownManager) {
    ...
    public String build() {
        ...
        text = player == null ? text : text.replaceAll("%cooldown_elytra%", cooldownManager.getTimer(CooldownType.ELYTRA_BOOST).getCooldown(player).toString());
        ...
        text = player == null ? text : text.replaceAll("%cooldown_trident%", cooldownManager.getTimer(CooldownType.TRIDENT_RIPTIDE).getCooldown(player).toString());
    ...
    private String timeToBars(Player player, CooldownType type) {
        PermissionManager pm = new PermissionManager(player, type);
        int totalDuration = pm.getGroupCooldown();
        if (totalDuration == 0) {
            return ChatColor.GREEN + "||||||||||";
        }
        int timeLeft = cooldownManager.getTimer(type).getCooldown(player);
